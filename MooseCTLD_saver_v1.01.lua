--[[%%%%% -= PERSISTENT WORLD SCRIPT  - Moose CTLD Version =- %%%%%
...
--%%%%% VARIABLES %%%%%
    MCTLDSave = {}
    MCTLDSave_deadUnitsTable = {}
    MCTLDSave_deadStaticsTable = {}
    MCTLDSave_spawnedUnitsTable = {}
    MCTLDSave_spawnedStaticsTable = {}
    MCTLDSave_marksTable = {}
    MCTLDSave_scoresTable = {} --Player,aa,ag,ash,tk,crash
    ...
    --> Dossier de sauvegarde (défaut : \Saved Games\DCS.Saves)
    ...
    local spawnedUnitsSaveFile = saveDir..saveFileName.."_MCTLDSave_spawned_units.lua"
    local spawnedStaticsSaveFile = saveDir..saveFileName.."_MCTLDSave_spawned_statics.lua"

--%%%%% TOOLKIT FUNCTIONS %%%%%
    --%%% DEBUG Log %%%
    local function DebugLog(msg)
        if DEBUG_MODE == true then
            Log("Moose CTLD Saver | "..msg)
            Msg("Moose CTLD Saver | "..msg,120)
        end
    end
    ...
    local function SpawnMe(data)
        ...
        --> NOT STATIC
            ...
            --> GROUND UNIT
                if data.objectSubCategory=="GROUND_UNIT" then
                    ...
                    Log("Moose CTLD Saver | Unit ".._data.units[1].type.." spawned")
            --> SHIP
                elseif data.objectSubCategory=="SHIP" then
        ...
        --> STATIC
            elseif data.objectCategory=="STATIC" then
                ...
                Log("Moose CTLD Saver | Static ".._data.type.." spawned")
        --> FARP
            elseif data.objectCategory=="BASE" then
                ...
                Log("Moose CTLD Saver | new FARP spawned")
    ...
    local function UpdateSpawnedUnitsTable()
        if saveBirthBlue == true or saveBirthRed == true then
            local _tempTable = {}
            for i = 1, #MCTLDSave_spawnedUnitsTable do
                if MCTLDSave_spawnedUnitsTable[i].coalition
                and MCTLDSave_spawnedUnitsTable[i].objectCategory
                and MCTLDSave_spawnedUnitsTable[i].objectSubCategory
                and MCTLDSave_spawnedUnitsTable[i].groupName
                and MCTLDSave_spawnedUnitsTable[i].units
                then
                    local _tempUnitsTable = {}
                    local _alive = 0
                    for i2=1, #MCTLDSave_spawnedUnitsTable[i].units do  
                        local _currentUnit = Unit.getByName(MCTLDSave_spawnedUnitsTable[i].units[i2].name)
                        if _currentUnit and _currentUnit:getLife() >= 1 then
                            ...
                            _tempUnitsTable[#_tempUnitsTable+1] = {
                                type = MCTLDSave_spawnedUnitsTable[i].units[i2].type,
                                name = MCTLDSave_spawnedUnitsTable[i].units[i2].name,
                                y = _currentPos.z,
                                x = _currentPos.x,
                                heading = _currentHeading,
                            }
                        else
                            DebugLog("Update Spawned : L'unité "..MCTLDSave_spawnedUnitsTable[i].units[i2].name.." n'existe plus",30)
                        end
                    end
                    if _alive > 0 then
                        _tempTable[#_tempTable+1] = {}
                        _tempTable[#_tempTable].coalition = MCTLDSave_spawnedUnitsTable[i].coalition
                        _tempTable[#_tempTable].objectCategory = MCTLDSave_spawnedUnitsTable[i].objectCategory
                        _tempTable[#_tempTable].objectSubCategory = MCTLDSave_spawnedUnitsTable[i].objectSubCategory
                        _tempTable[#_tempTable].groupName = MCTLDSave_spawnedUnitsTable[i].groupName
                        _tempTable[#_tempTable].units = _tempUnitsTable
                    else
                        DebugLog("Update Spawned : L'unité "..MCTLDSave_spawnedUnitsTable[i].groupName.." n'existe plus",30)
                    end -- group alive
                else
                    DebugLog("Update Spawned : 1 loop skipped, a nil value ",5)
                end -- group exist
            end
            MCTLDSave_spawnedUnitsTable = _tempTable
            DebugLog("Update Spawned complete ("..#MCTLDSave_spawnedUnitsTable.. ")")
        end
    end
    ...
    local function UpdateSpawnedStaticsTable()
        if saveStaticsBirthBlue == true or saveStaticsBirthRed == true then
            local _tempTable = {}
            for i = 1, #MCTLDSave_spawnedStaticsTable do
                if MCTLDSave_spawnedStaticsTable[i].objectCategory
                and MCTLDSave_spawnedStaticsTable[i].coalition
                and MCTLDSave_spawnedStaticsTable[i].name
                and MCTLDSave_spawnedStaticsTable[i].type
                and MCTLDSave_spawnedStaticsTable[i].category
                and MCTLDSave_spawnedStaticsTable[i].shape
                and MCTLDSave_spawnedStaticsTable[i].x
                and MCTLDSave_spawnedStaticsTable[i].y
                then
                    local _alive = 0 
                    local _currentUnit = StaticObject.getByName(MCTLDSave_spawnedStaticsTable[i].name)
                    DebugLog("Statics : "..MCTLDSave_spawnedStaticsTable[i].name)
                    if _currentUnit and _currentUnit:getLife() >= 1 then
                        _alive = _alive+1
                    else
                        DebugLog("Update spawned statics : L'unité "..MCTLDSave_spawnedStaticsTable[i].name.." n'existe plus",30)
                    end
                    if _alive > 0 then
                        _tempTable[#_tempTable+1] = {}
                        _tempTable[#_tempTable].objectCategory = MCTLDSave_spawnedStaticsTable[i].objectCategory
                        _tempTable[#_tempTable].coalition = MCTLDSave_spawnedStaticsTable[i].coalition
                        _tempTable[#_tempTable].name = MCTLDSave_spawnedStaticsTable[i].name
                        _tempTable[#_tempTable].type = MCTLDSave_spawnedStaticsTable[i].type
                        _tempTable[#_tempTable].category = MCTLDSave_spawnedStaticsTable[i].category
                        _tempTable[#_tempTable].shape = MCTLDSave_spawnedStaticsTable[i].shape
                        _tempTable[#_tempTable].x = MCTLDSave_spawnedStaticsTable[i].x
                        _tempTable[#_tempTable].y = MCTLDSave_spawnedStaticsTable[i].y
                        _tempTable[#_tempTable].heading = MCTLDSave_spawnedStaticsTable[i].heading
                    else
                        DebugLog("Update spawned statics : L'unité "..MCTLDSave_spawnedStaticsTable[i].name.." n'existe plus",30)
                    end
                else
            ...
            MCTLDSave_spawnedStaticsTable = _tempTable
            DebugLog("Update spawned statics complete ("..#MCTLDSave_spawnedStaticsTable.. ")")
        end
    end
    ...
    local function SaveSpawnedUnits(timeloop, time)
        ...
        local _spawnedStr = SmartSerialize("MCTLDSave_spawnedUnitsTable", MCTLDSave_spawnedUnitsTable)
        WriteSaveData(_spawnedStr, spawnedUnitsSaveFile)
        Log("Moose CTLD Saver | Spawned units Saved ("..#MCTLDSave_spawnedUnitsTable..")")
    ...
    local function SaveSpawnedStatics(timeloop, time)
        ...
        local _spawnedStaticsStr = SmartSerialize("MCTLDSave_spawnedStaticsTable", MCTLDSave_spawnedStaticsTable)
        WriteSaveData(_spawnedStaticsStr, spawnedStaticsSaveFile)
        Log("Moose CTLD Saver | Spawned statics Saved ("..#MCTLDSave_spawnedStaticsTable..")")
        return time + saveSchedule
    end
...
--%%%%% MAIN () %%%%%
    ...
    --> Loading message
    Msg("Moose CTLD Saver | Loading...  -  Credits : JGi | Quéton 1-1", 5)
    Log("Moose CTLD Saver | Loading...  -  Credits : JGi | Quéton 1-1")
    Log("Moose CTLD Saver | Save folder : "..saveDir)
    ...
    if os ~= nil then
        ...
        if saveBirthBlue == true or saveBirthRed == true then
            if FileExists(spawnedUnitsSaveFile) then	
                Log("Moose CTLD Saver | Found spawned units save file")	
                dofile(spawnedUnitsSaveFile)
                restoredUnits = 0
                if MCTLDSave_spawnedUnitsTable then   
                    for i = 1, #MCTLDSave_spawnedUnitsTable do
                        
                        if MCTLDSave_spawnedUnitsTable[i]
                        and MCTLDSave_spawnedUnitsTable[i].objectCategory
                        and MCTLDSave_spawnedUnitsTable[i].objectSubCategory
                        and MCTLDSave_spawnedUnitsTable[i].coalition
                        and MCTLDSave_spawnedUnitsTable[i].groupName
                        and MCTLDSave_spawnedUnitsTable[i].units
                        then
                            local _data = {}
                            if MCTLDSave_spawnedUnitsTable[i].coalition == 2 then _coalition = country.id.CJTF_BLUE else _coalition = country.id.CJTF_RED end
                            if MCTLDSave_spawnedUnitsTable[i].objectCategory==1 and MCTLDSave_spawnedUnitsTable[i].objectSubCategory==2 then 
                                _data.objectCategory="UNIT"
                                _data.objectSubCategory="GROUND_UNIT"
                                _data.groupName = MCTLDSave_spawnedUnitsTable[i].groupName
                                _data.x=MCTLDSave_spawnedUnitsTable[i].units[1].x
                                _data.y=MCTLDSave_spawnedUnitsTable[i].units[1].y
                                _data.units={}
                                for y=1, #MCTLDSave_spawnedUnitsTable[i].units do
                                    _data.units[#_data.units+1]={
                                        type=MCTLDSave_spawnedUnitsTable[i].units[y].type,
                                        x = MCTLDSave_spawnedUnitsTable[i].units[y].x,
                                        y = MCTLDSave_spawnedUnitsTable[i].units[y].y,
                                        heading = MCTLDSave_spawnedUnitsTable[i].units[y].heading,
                                    }
                                end
                            ...
                            Log("Moose CTLD Saver | Restoring unit : One loop skip, a nil value")
                        end	
                    end
                    Log("Moose CTLD Saver | Restored "..restoredUnits.." Unit(s)")
                end
            else
                Log("Moose CTLD Saver | No Spawned save file")			
            end
        end
        ...
        if saveStaticsBirthBlue == true or saveStaticsBirthRed == true then
            if FileExists(spawnedStaticsSaveFile) then	
                Log("Moose CTLD Saver | Found spawned statics save file")	
                dofile(spawnedStaticsSaveFile)
                restoredStatics = 0
                if MCTLDSave_spawnedStaticsTable then
                    for i = 1, #MCTLDSave_spawnedStaticsTable do
                        if MCTLDSave_spawnedStaticsTable[i]
                        and MCTLDSave_spawnedStaticsTable[i].objectCategory
                        and MCTLDSave_spawnedStaticsTable[i].coalition
                        and MCTLDSave_spawnedStaticsTable[i].name
                        and MCTLDSave_spawnedStaticsTable[i].type
                        and MCTLDSave_spawnedStaticsTable[i].category
                        ...
                        then
                            local _data = {}
                            if MCTLDSave_spawnedStaticsTable[i].coalition == 2 then _coalition = country.id.CJTF_BLUE else _coalition = country.id.CJTF_RED end
                            if MCTLDSave_spawnedStaticsTable[i].objectCategory==3 then 
                                _data.objectCategory="STATIC"
                            elseif MCTLDSave_spawnedStaticsTable[i].objectCategory==4 then
                                _data.objectCategory="BASE"
                                _data.freq=225.1
                            end
                            _data.groupName = MCTLDSave_spawnedStaticsTable[i].name
                            _data.units={{
                                type=MCTLDSave_spawnedStaticsTable[i].type,
                                category = MCTLDSave_spawnedStaticsTable[i].category,
                                shape = MCTLDSave_spawnedStaticsTable[i].shape,
                            }}
                            _data.x = MCTLDSave_spawnedStaticsTable[i].x
                            _data.y = MCTLDSave_spawnedStaticsTable[i].y
                            _data.heading = MCTLDSave_spawnedStaticsTable[i].heading
                            SpawnMe(_data)
                            restoredStatics = restoredStatics + 1
                        else
                            Log("Moose CTLD Saver | Restoring statics : One loop skip, a nil value")
                        end	
                    end
                    Log("Moose CTLD Saver | Restored "..restoredStatics.." Static(s)")
                end
            else
                Log("Moose CTLD Saver | No Spawned statics save file")        
            end
        end
        ...
        --%%% EVENT LOOP - ON BIRTH %%%
            MCTLDSave.onBirthEventHandler = {}
            function MCTLDSave.onBirthEventHandler:onEvent(Event)
                    ...
                    if Event.initiator then
                            ...
                            if (birthCoalition == 1 and saveBirthRed == true or birthCoalition == 2 and saveBirthBlue == true) then
                                if birthObjectCategory == 1 and birthUnitCategory == 2 then -- UNIT
                                    ...
                                    if _match ~=0 or _pMatch ~=0 then  
                                        Log("Moose CTLD Saver | Birth Unit ignored")
                                    else
                                        local groupMatch = 0
                                        for i = 1, #MCTLDSave_spawnedUnitsTable do
                                            if MCTLDSave_spawnedUnitsTable[i].groupName == birthGroupName then
                                                groupMatch = groupMatch+1
                                                MCTLDSave_spawnedUnitsTable[i].units[#MCTLDSave_spawnedUnitsTable[i].units+1] = {
                                                    type = birthUnitType,
                                                    name = birthUnitName,
                                                    y = birthUnitPosY,
                                                    x = birthUnitPosX,
                                        ...
                                        if groupMatch == 0 then
                                            MCTLDSave_spawnedUnitsTable[#MCTLDSave_spawnedUnitsTable+1] = {
                        ...
                        elseif Object.getCategory(Event.initiator) == 4 and Event.initiator:getCoalition() and savedTypesToStaticBirthList[Event.initiator:getDesc().typeName]
                            ...
                            if (Event.initiator:getCoalition() == 1 and saveStaticsBirthRed == true or Event.initiator:getCoalition() == 2 and saveStaticsBirthBlue == true) then
                                MCTLDSave_spawnedStaticsTable[#MCTLDSave_spawnedStaticsTable+1] = {
            ...
            world.addEventHandler(MCTLDSave.onBirthEventHandler)
    else
        Msg("Moose CTLD Saver | Error, MissionScripting.lua 'sanitize'.", 10)
        Log("Moose CTLD Saver | Error, MissionScripting.lua 'sanitize'.")
    end